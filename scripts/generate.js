...
// import { markDeviationUsed } from "../lib/concepts/selectDeviations.js";
// import { markLensUsed } from "../lib/concepts/selectLenses.js";
...
//       const { concepts, anchor, deviation, lens } = await generateConceptSeeds(
//         key
//       );

//       if (!anchor || !deviation || !lens) {
...
//         if (!item.axis_id) {
//           console.warn(
//             `⚠️ Skipping concept: missing axis_id → "${item.concept.slice(
//               0,
//               80
//             )}..."`
//           );
...
//         let score = 50; // safe fallback
//         try {
//           score = await scoreConceptWow(item.concept, key);
//         } catch (err) {
//           console.warn(
//             `⚠️ WOW-scoring failed, using fallback (50) for concept: "${item.concept.slice(
//               0,
//               80
//             )}..."`
//           );
...
//         console.log("🧠 PICKED STRUCTURE", {
//           axis_id: selectedAxisId,
//           anchor_id: anchor.id,
//           deviation_id: deviation.id,
//           lens_id: lens.id,
//           wow: selectedWowScore,
//         });
...
//       // ==================================================================
//       if (!selectedAxisId || !anchor?.id || !deviation?.id || !lens?.id) {
...
//       // ==================================================================
//       await Promise.all([
//         selectedAxisId && markAxisUsed(selectedAxisId),
//         anchor?.id && markAnchorUsed(anchor.id),
//         deviation?.id && markDeviationUsed(deviation.id),
//         lens?.id && markLensUsed(lens.id),
//       ]);
...
export async function main() {
  ...
  try {
    ...
    for (const [key, { tone, image }] of Object.entries(categories)) {
      ...
      // MARK STRUCTURE AS USED — ONLY AFTER SUCCESSFUL SAVE
      // ==================================================================
      await Promise.all([anchor?.id && markAnchorUsed(anchor.id)]);
