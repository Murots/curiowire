...
//     "pick_random_unused_unflagged_list",
//   );
...
//   return {
//     table: "curiosity_list_suggestions",
//     row,
//   };
// }
// // async function fetchOneListSuggestion() {
...
async function resolveListSourceUrls({
  title,
  summary_normalized,
  category,
  maxAttempts = 3,
  ...
  const finalUrls = [...new Set(accepted)].slice(0, 3);

  console.log(`🔗 List SourceResolver: ${finalUrls.length ? "FOUND" : "NONE"}`);

  if (finalUrls.length) {
    console.log(`   source_urls=${finalUrls.join(" | ")}`);
  }
