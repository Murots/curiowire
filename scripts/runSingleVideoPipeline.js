...
// Runs ONE queued video job: generate → post
...
async function run() {
  ...
  // --------------------------------------------------------------------------
  // 4) Post video to platforms
  // --------------------------------------------------------------------------
  console.log("🚀 Posting video...");

  await updateJob(job.id, { status: "posting" });

  const results = await postVideoToPlatforms({
    ...job,
  ...
  // --------------------------------------------------------------------------
  // 5) Final status validation (extra safety)
  // --------------------------------------------------------------------------
  ...
  if (!anySuccess) {
    console.log("❌ No platform succeeded — marking job as failed");

    await updateJob(job.id, {
      status: "failed",
    });

    throw new Error("All platform uploads failed");
  }
