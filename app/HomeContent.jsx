...
// function getVideoPublishAtMs(video) {
//   const raw = video?.posted_results?.youtube?.detail?.publishAt;
//   const ms = raw ? new Date(raw).getTime() : NaN;
//   return Number.isFinite(ms) ? ms : NaN;
// }

// function isLiveYoutubeVideo(video) {
//   const publishAtMs = getVideoPublishAtMs(video);
...
//     attachLiveVideoToCardRows(initialCards || []),
//   );
...
//       setCards(attachLiveVideoToCardRows(saved.cards || []));
...
//         const items = attachLiveVideoToCardRows(
...
//           const nextData = dedupeById(attachLiveVideoToCardRows(data || []));

//           if (!error) {
//             if (page === 1) {
//               setCards(nextData);
//             } else {
//               setCards((prev) => dedupeById([...(prev || []), ...nextData]));
//             }
...
//           .select(
//             "id, category, title, summary_normalized, image_url, created_at, wow_score, article_type, quote_text, videos(youtube_video_id, youtube_url, posted_at, posted_results, status)",
//           )
//           .eq("status", "published")
//           .eq("is_listed", true);
...
//     setCards(dedupeById(attachLiveVideoToCardRows(filteredBySearch)));
//     setHasMore(false);
...
//         const ids = baseRows.map((x) => Number(x?.id)).filter(Boolean);
...
//         const { data: liveVideos } = await supabase
...
//           if (!byArticleId.has(articleId)) byArticleId.set(articleId, []);
//           byArticleId.get(articleId).push({
...
//           ...row,
//           videos: byArticleId.get(Number(row?.id)) || [],
//         }));

//         setCards(dedupeById(merged));
//         setHasMore(false);
//       } catch {
//         if (!alive) return;
//         setCards([]);
...
//             : sortQ === "quote"
//               ? "No quote curiosities found."
//               : sortQ === "video"
//                 ? "No video curiosities found."
...
//                 <option value="quote">Quote Curios</option>
...
//               {firstInterrupt ? (
//                 <FeedInterrupt
//                   block={firstInterrupt}
//                   videoItems={
//                     firstInterrupt.type === "video_carousel" &&
//                     !videoItemsLoading
//                       ? videoItems
//                       : []
//                   }
...
//                   videoItems={
//                     secondInterrupt.type === "video_carousel" &&
//                     !videoItemsLoading
//                       ? videoItems
//                       : []
//                   }
...
export default function HomeContent({ initialCards, initialQuery }) {
  ...
  const [oldGemsItems, setOldGemsItems] = useState([]);
  const [oldGemsItemsLoading, setOldGemsItemsLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    async function loadOldGems() {
      setOldGemsItemsLoading(true);

      try {
        const res = await fetch("/api/old-gems", {
          cache: "no-store",
        });
        ...
        setOldGemsItems(Array.isArray(json?.items) ? json.items : []);
      } catch {
        if (!alive) return;
        setOldGemsItems([]);
      } finally {
        if (alive) setOldGemsItemsLoading(false);
      }
    }
  ...
  const feedSegments = useMemo(() => {
    ...
    if (!eligibleInterrupts.length) {
      return [{ type: "cards", items: cards }];
    }

    const segments = [];
    let cursor = 0;

    eligibleInterrupts.forEach((block) => {
      const insertAfter = Number(block?.insertAfter || 0);
      const nextCursor = Math.min(insertAfter, cards.length);

      if (nextCursor > cursor) {
        segments.push({
          type: "cards",
          items: cards.slice(cursor, nextCursor),
        });
      }
    ...
    if (cursor < cards.length) {
      segments.push({
        type: "cards",
        items: cards.slice(cursor),
      });
    }

    return segments;
  }, [cards, eligibleInterrupts]);
            ...
            <>
              {feedSegments.map((segment, index) => {
                ...
                if (segment.type === "interrupt") {
                  ...
                  return (
                    <FeedInterrupt
                      key={`interrupt-${segment.block?.id || index}`}
                      block={segment.block}
                      videoItems={
                        blockType === "video_carousel" && !videoItemsLoading
                          ? videoItems
                          : []
                      }
                      oldGemsItems={
                        blockType === "old_gems" && !oldGemsItemsLoading
                          ? oldGemsItems
                          : []
                      }
                    />
