...
- Each hook MUST be based ONLY on the MAIN FACT below.
- Do NOT add new facts, numbers, names, places, causes, or conclusions.
- Must be 100% factual and directly supported by the MAIN FACT.
...
1) CONCRETE ANCHOR:
Reference something specific and tangible (e.g., a measurable quantity, a defined time/era,
or a clearly identifiable thing), so the brain can “grab onto” it immediately.

2) EXPECTATION BREAK:
Signal that the situation does not behave as normally expected, conflicts with common assumptions,
or contradicts intuitive understanding.

3) CLEAR SUBJECT:
Explicitly name the main object, system, or entity involved so the viewer instantly knows
what the sentence is about. Avoid vague or contextless references.

4) OPEN LOOP:
Imply that important information is missing or unresolved, creating curiosity that can only be
satisfied by continuing the video—without revealing the conclusion or twist.

STYLE:
- Short, punchy, spoken rhythm.
- Low cognitive load: avoid complex punctuation or long technical words.
- Specific > generic.

MAIN FACT (the only allowed source):
"${mainFact}"
