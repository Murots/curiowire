...
//
// Pipeline:
// model (PikaScenes/PikaText/KlingText) → slowdown
// → thumbnail-frame before intro → intro/outro → TTS+FX → BGM → SFX → subs → final mux
// FFmpeg-2018 compatible
//
// vNext update (fallback):
// - If provider=kling: try Kling first
...
// - On Scenes failure: fallback to Pika Text
// - If no anchor: skip scenes and go directly to Pika Text
// - ✅ Thumbnail is NO LONGER prepended to the AI clip
// - ✅ Thumbnail is now inserted at the absolute START of the full video (before intro)
// - ✅ Audio timing remains restored to original intro timing
// ============================================================================
...
// Restored original timing because the thumbnail segment is no longer inside the AI clip
const BULB_DELAY_SECONDS = 0.1;
const VOICE_DELAY_SECONDS = 2.35;
...
`.trim();
  ...
  async function pipelineWithBuffer(videoBuffer, label = "MODEL") {
    ...
    // Generate thumbnail image used as first frame(s) of the FULL video (before intro)
    let thumbnailImagePath = null;
    try {
      const thumbnail = await createYouTubeThumbnail(article);
      thumbnailImagePath = thumbnail?.path || null;

      if (thumbnailImagePath) {
        console.log(
          `🖼 Thumbnail frame ready for full-video start: ${thumbnailImagePath} | hook="${thumbnail.hook}"`,
        );
      }
    } catch (err) {
      console.warn("⚠️ Thumbnail frame generation failed:", err.message);
      thumbnailImagePath = null;
    }

    // Slowdown + vertical shaping
    const slowPath = await slowDownVideoToVertical(rawPath, 0.5);
    console.log("🐌 Slowed + vertical video:", slowPath);
    await logVideoInfo(`${label} SLOW+VERTICAL`, slowPath);

    // Add thumbnail at absolute start, then intro, then main, then outro
    const introPath = "./lib/video/intro/intro.mp4";
    console.log("🎬 Adding thumbnail + intro + outro around main video…");
    const videoWithIntroOutro = await addIntroAndOutro(
      introPath,
      slowPath,
      thumbnailImagePath,
    );
    console.log(
      "🎬 Thumbnail+intro+main+outro video (no audio yet):",
      videoWithIntroOutro,
    );
    await logVideoInfo(`${label} INTRO+OUTRO`, videoWithIntroOutro);
    ...
    console.log(
      "✅ FULL-AI final video with thumbnail-before-intro + intro/outro + SFX + delayed audio + subs:",
      finalPath,
    );
