...
// v4.2 — Provider toggle (Pika/Kling) + env-safe (NO import-time caching)
//
// Guarantees (always):
// - duration: 10 seconds
// - aspect_ratio: 9:16
...
// - KLING:
//    text:               prompt + aspect_ratio + duration (+ cfg_scale + generate_audio=false)
//
// Env toggles:
...
//   KLING_TEXT_MODEL="fal-ai/kling-video/v3/pro/text-to-video"
//
// Kling optional:
...
const LOCKED_DURATION_SECONDS = 10;
...
function normalizePikaDurationTo510(duration) {
...
function normalizeKlingDuration(duration) {
  ...
  if (!Number.isFinite(d)) return "10";
  return String(Math.min(15, Math.max(3, d)));
}
...
function getEnvDefaults() {
  ...
  // Pika resolution default (Kling schema does not use resolution here)
...
export async function callPikaAPI({
  ...
  if (effProvider === "kling") {
    ...
    const klingDuration = normalizeKlingDuration(LOCKED_DURATION_SECONDS);
    loggedDuration = klingDuration;
    ...
    const pikaDuration = normalizePikaDurationTo510(LOCKED_DURATION_SECONDS);
    loggedDuration = String(pikaDuration);
    ...
    // For schema safety: image-to-video did not list aspect_ratio
    if (!hasImage) {
  ...
  console.log(`⏱ duration=${loggedDuration}s | aspect_ratio=${aspectRatio}`);
  if (effProvider === "pika") {
    console.log(`📺 resolution=${inputObj.resolution}`);
  }
  if (effProvider === "kling") {
    console.log(`🎛 cfg_scale=${inputObj.cfg_scale}`);
    console.log(`🔇 generate_audio=${inputObj.generate_audio}`);
  }
