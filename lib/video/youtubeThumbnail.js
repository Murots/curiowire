...
function wrapTitle(title, maxLineLength = 15, maxLines = 3) {
  ...
  // Smartere layout for 4 ord
  if (titleWords.length === 4) {
    const a = `${titleWords[0]} ${titleWords[1]}`;
    if (fits(a)) {
      return [a, `${titleWords[2]}`, `${titleWords[3]}`];
    }
  ...
  // Smartere layout for 5 ord
  if (titleWords.length === 5) {
    const a = `${titleWords[0]} ${titleWords[1]}`;
    const b = `${titleWords[2]} ${titleWords[3]}`;

    if (fits(a) && fits(b)) {
      return [a, b, `${titleWords[4]}`];
    }

    if (fits(a)) {
      return [a, `${titleWords[2]}`, `${titleWords[3]} ${titleWords[4]}`];
    }

    return [
      `${titleWords[0]}`,
      `${titleWords[1]} ${titleWords[2]}`,
      `${titleWords[3]} ${titleWords[4]}`,
    ].map((line) => line.trim());
  }
...
async function selectBestHook(card) {
  function fitHookForLayout(rawHook) {
    ...
    const fiveLines = wrapTitle(fiveDisplay, 15, 3);
    ...
    const fourLines = wrapTitle(fourDisplay, 15, 3);
...
function buildOverlaySvg(hook, category, signalColor) {
  const lines = wrapTitle(hook, 15, 3);
