...
export async function generateConceptSeeds(category) {
  try {
    ...
    console.log(`🧭 Axis pool:\n${axes.map((a) => "• " + a.axis).join("\n")}`);
    console.log(`⚓ Anchor: ${anchor.anchor}`);
    console.log(`🧨 Deviation: ${deviation.deviation}`);
    console.log(`🔍 Lens: ${lens.lens}`);

    // ------------------------------------------------------------------------
    // STEP 1 — Chaos seeds (unchanged)
    // ------------------------------------------------------------------------
    let seedWords = await generateChaosSeeds(category);
    seedWords = seedWords.filter((w) => !w.includes(" "));
...
DEVIATION (EXPECTATION BREAK):
${deviation.deviation}

LENS (INTERPRETATION MODE):
${lens.lens}
...
• Each concept MUST clearly depend on:
  - the anchor
  - the deviation
  - the lens
  - ONE axis only
• No people unless structurally necessary
