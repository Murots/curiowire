...
function isCentralEntity(name, title, article, type = "person") {
  ...
  // === Country rule: more tolerant (often structural)
  if (type === "country") {
...
// 3) Determine if entity is PRIMARY SUBJECT or merely SUPPORTING / CONTEXTUAL
async function isEntityPrimary(openai, name, title, article) {
  const prompt = `
Is "${name}" the PRIMARY SUBJECT of this article,
or is it only a SUPPORTING or CONTEXTUAL reference?

Answer ONLY with:
- "primary" → the article is fundamentally ABOUT this entity
- "context" → the entity is mentioned to support or situate another topic
