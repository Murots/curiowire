...
function pickImageCaption(cand) {
  if (!cand || cand.provider === "DALL·E") return null;

  if (cand.provider === "Wikimedia") {
    return (
      cleanCaptionText(cand.description) || cleanCaptionText(cand.title) || null
    );
  }

  if (cand.provider === "Pexels") {
    return cleanCaptionText(cand.alt) || null;
  }

  if (cand.provider === "Unsplash") {
    return (
    ...
    if (gptWinner) {
      ...
      if (cached) {
        return {
          ...
          artist: gptWinner.artist,
          attribution: buildAttribution(gptWinner),
          caption: pickImageCaption(gptWinner),
          selectedBy: "gpt-final-merge",
          selectedLane: gptWinner.lane,
          selectedQuery: gptWinner.queryUsed,
        };
