...
// ============================================================================================
// 👤 CENTRAL ENTITY DETECTION (person eller brand)
// ============================================================================================

// 1) Detect name via ultra-cheap GPT-mini NER
async function detectMainEntity(openai, title, article) {
  const prompt = `
...
{
  "type": "person" | "brand" | "country" | "city" | "landmark" | "none",
...
// 2) Check if entity is really central
function isCentralEntity(name, title, article, type = "person") {
  ...
  const freq = (a.match(new RegExp(n, "g")) || []).length;

  // === Country rule: more tolerant ===
  if (type === "country") {
  ...
  // === Cities and landmarks follow same logic as person/brand ===
...
// 3) OPTIONAL: detect if entity is described as main actor (cheap GPT check)
async function isEntityPrimary(openai, name, title, article) {
  const prompt = `
Is "${name}" the MAIN SUBJECT of this article, or only a supporting figure?
Answer ONLY with: "primary" or "supporting".
...
`;
  ...
  try {
    ...
    const ans = r.choices[0].message.content.toLowerCase();
    return ans.includes("primary");
  } catch {
    return false;
  }
}
...
export async function getCoreNoun(title, article) {
  ...
  const entityData = await detectMainEntity(openai, title, article);
  const entity = entityData.name;
  ...
  if (
    ...
    const primary = await isEntityPrimary(openai, entity, title, article);
    if (primary) {
      const clean = entity.toLowerCase().replace(/[^a-z0-9\s]/g, "");
      ...
      // === PERSON or BRAND
      console.log(`🎯 Person/brand detected → "${clean}"`);
      return clean;
    }
  }
