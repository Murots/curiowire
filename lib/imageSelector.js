...
// {
//   "type": "person" | "brand" | "country" | "city" | "landmark" | "polity"${category === "space" ? ` | "phenomenon"` : ""} | "none",
...
//   // Country / polity: tolerant (often appears structurally)
...
//       // === POLITY HANDLING: return entity name directly
...
export async function selectBestImage(
  ...
  // ------------------------------------------------------------------------
  // E) Fallback → bruk top-1 merged kandidat
  // ------------------------------------------------------------------------
  if (finalists.length) {
    ...
    console.log(
      `🏆 Fallback selected (top-1 merged) [${best.lane}] ${best.provider} (${best.score}) → ${best.url}`,
    );
